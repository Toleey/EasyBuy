...
public class OrderServiceImpl implements OrderService {
    ...
    public Order payShoppingCart(ShoppingCart cart, User user, String address) {
        ...
        try {
            ...
            session.getMapper(OrderDao.class).add(order);
...
//        try {
//            session = MyBatisUtil.createSession();
//            orderList = session.getMapper(OrderDao.class).getOrderList(
//                    userId, (currentPageNo - 1) * pageSize, pageSize);
//            for (Order order : orderList) {
//                order.setOrderDetailList(session.getMapper(
//                        OrderDetailDao.class).getOrderDetailList(order.getId()));
//                for (OrderDetail item : order.getOrderDetailList()) {
//                    item.setProduct(session.getMapper(ProductDao.class)
